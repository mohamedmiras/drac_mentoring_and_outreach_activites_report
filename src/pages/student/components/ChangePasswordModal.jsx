...
const ChangePasswordModal = ({ isOpen, onClose, studentId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const studentSnap = await getDocs(query(collection(db, 'students'), where('admissionNumber', '==', studentId))); // wait, studentId passed here might be admissionNumber
      
      // Better approach: use the document ID if available, or fetch by admissionNumber
      const q = query(collection(db, 'students'), where('admissionNumber', '==', studentId));
      const querySnapshot = await getDocs(q);
      
      if (querySnapshot.empty) throw new Error('Student data not found.');
